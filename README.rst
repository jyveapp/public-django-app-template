...
A new public Django app can be started with::
    ...
    pip3 install temple
    temple setup git@github.com:jyveapp/public-django-app-template.git
...
5. Automatic generation of CHANGELOG.md using
   `git-tidy <https://github.com/jyveapp/git-tidy>`__.
...
click "Jyve Corporation", and refresh the repositories. Import the
...
ones at Jyve. If there is an error in the templated files or a change that
...
``temple ls git@github.com:jyveapp/public-django-app-template.git`` to see a
list of all projects spun up with this template for examples at Jyve.
...
other Django apps at Jyve, all the way from local development to
...
at Jyve, such as when we upgrade Python to newer versions or potentially
